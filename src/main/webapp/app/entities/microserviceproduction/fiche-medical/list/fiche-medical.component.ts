...
export class FicheMedicalComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.ficheMedicalService
      ...
      .subscribe(
        (res: HttpResponse<IFicheMedical[]>) => {
  ...
  protected onSuccess(data: IFicheMedical[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/fiche-medical'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
